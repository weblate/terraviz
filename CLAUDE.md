...
| `src/services/photorealEarth.ts` | Reusable photoreal Earth factory — diffuse / night lights / specular / atmosphere / clouds / sun / ground shadow with day/night shading; shared by VR view and Orbit character page. Also the **texture provider** for the multi-monitor output (rung 12c), which has no mesh to hang a material on: `nightLightsTexture` / `onNightLightsChange` and `cloudTexture` / `onCloudChange` mirror the `baseDiffuseTexture` / `onBaseDiffuseChange` pair, so a consumer compositing for itself tracks every tier this stack loads instead of running a second loader against the same CDN. Cloud **alpha is coverage** — the loader bakes it from luminance on a canvas before upload, so a consumer reads `.a` and must not repeat the gamma |
