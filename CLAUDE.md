...
> dataset's media and steers its playhead. **What it does not yet do is
> composite that media onto the sphere** — `buildOutputFragmentShader`
> exists and is tested, but `outputScene` still renders the base Earth
> pass alone, so a mirrored dataset is loaded, synced and unseen. So the modules above are description,
